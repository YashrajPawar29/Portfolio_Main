      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {skillCategories.map((category, index) => (
            <div
              key={index}
              className="group bg-slate-700/30 rounded-2xl p-8 border border-slate-600/50 hover:border-emerald-500/30 transition-all duration-300 hover:shadow-xl hover:shadow-emerald-500/5"
            >
              {/* Category header */}
              <div className="flex items-center gap-4 mb-6">
                <div className="w-12 h-12 bg-emerald-500/10 rounded-xl flex items-center justify-center group-hover:bg-emerald-500/20 transition-colors">
                  <category.icon size={22} className="text-emerald-400" />
                </div>
              ...
              {category.type === "progress" ? (
                ...
                <div className="flex flex-wrap gap-3">
                  {category.skills.map((skill, idx) => (
                    <span
                      key={idx}
                      className="px-4 py-2 bg-slate-600/40 text-slate-300 rounded-lg text-sm hover:bg-emerald-500/20 hover:text-emerald-300 transition-colors cursor-default"
                    >
                      {skill}
                    </span>
                  ))}
